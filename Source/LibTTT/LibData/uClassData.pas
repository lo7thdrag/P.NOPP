...
type
  ...
  // File Transfer (Tote Display)
  TFileTransferData = class
  private
    FFileName   : string;
    FFileSize   : Int64;
    FOwnerIP    : string;
  ...
  TFileSendTransferContainer  = class
  private
    FFileSendList : TObjectList;
    FFileStream   : TFileStream;
    FCurrentFile  : string;
...
begin
  ...
  begin
    ...
    {$REGION ' Rotation Font '}

    rotationTemp := round( 10.0 * (90.0 - heading ));
    headingTemp := DegToRad(90.0 - heading);

    tf := TFont.Create;
    tf.Assign(Font);
    GetObject(tf.Handle, sizeof(lf), @lf);

    lf.lfEscapement  :=  rotationTemp;
    lf.lfOrientation :=  rotationTemp;
    ...
    sz := TextExtent(simbol);

    SetBkMode(Handle, TRANSPARENT);
    SinCos(headingTemp , sinX, cosX );

    qx := cx - Floor( 0.5 * sz.cy * sinX);
    qy := cy - Floor( 0.5 * sz.cy * cosX);

    TextOut(qx, qy,simbol);
    ...
    {$REGION ' Normalization Rotation Font '}

    rotationTemp := Round( 10.0 * (90.0 - 90 ));
    headingTemp := DegToRad(90.0 - 90);
    ...
    TextOut(round(cx), round(cy+10), Identifier);
...
{$REGION 'File Send Transfer Container'}

procedure TFileSendTransferContainer.AddFile(FileData: TFileTransferData);
begin
 if not Assigned(FileData) then
    Exit;
...
constructor TFileSendTransferContainer.Create;
begin
  inherited;
...
destructor TFileSendTransferContainer.Destroy;
begin
  if Assigned(FFileStream) then
  begin
    FFileStream.Free;
...
procedure TFileSendTransferContainer.RemoveFile(FileData: TFileTransferData);
begin
  if Assigned(FileData) then
    FFileSendList.Remove(FileData);
end;
...
{$REGION 'File Transfer Data'}

constructor TFileTransferData.Create;
begin
  inherited;
...
destructor TFileTransferData.Destroy;
begin
  if Assigned(FStream) then
    FStream.Free;
